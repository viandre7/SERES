function limpiar() {
    $("#txtNombreCat").val("");
    $("#txtPrecioCat").val("");
}

function eliminar(id_comentario, comentario, estado){
    $("#txtComEli").val(id_comentario);
    $("#comentarioElim").html(comentario);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
}
    
$(document).ready(function(){
    buscar(); 

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/comentario.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class=' caption-top table  border-primary compact dt-responsive nowrap table-striped table-hover border rounded '>";
            datos += '<caption>Lista de Comentarios</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="text-center align-middle border border-primary " >Estado</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Oferta</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Ofertante</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Comentario</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Calificación</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Usuario que Califico</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Fecha Calificación</th>';
                        datos += '<th class="text-center align-middle border border-primary " scope="col" >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                        if(value.estado=="A"){
                            datos += '<td  class=" border border-primary activo text-wrap">Activo</td>';
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.nombre_oferta+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.primer_nombre+' '+value.otros_nombre+' '+value.primer_apellido+' '+value.otros_apellido+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.comentario+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.calificacion+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.usuario+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.fecha_registro+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_servicio+',\''+value.comentario+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                        }else{
                            datos += '<td class=" border border-primary inactivo text-wrap" >Inactivo</td>';
                            datos += '<td class=" border border-primary inactivo text-wrap" >'+value.nombre_oferta+'</td>'; 
                            datos += '<td class=" border border-primary inactivo text-wrap" >'+value.primer_nombre+' '+value.otros_nombre+' '+value.primer_apellido+' '+value.otros_apellido+'</td>'; 
                            datos += '<td class=" border border-primary inactivo text-wrap" >'+value.comentario+'</td>'; 
                            datos += '<td class=" border border-primary inactivo text-wrap" >'+value.calificacion+'</td>'; 
                            datos += '<td class=" border border-primary inactivo text-wrap" >'+value.usuario+'</td>'; 
                            datos += '<td class=" border border-primary inactivo text-wrap" >'+value.fecha_registro+'</td>'; 
                            datos += '<td class=" border border-primary inactivo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_servicio+',\''+value.comentario+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                        }
                        datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[6, 'desc']],    
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtilp',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        title: 'Comentarios',
                        titleAttr: 'PDF',
                        className: 'btn-primary',
                        messageTop: 'Comentarios',
                        messageTop: 'PDF con la información de los Comentarios.',
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            columns: [0,1,2,3,4,5,6]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                              } );
                        }
                    },
                    {
                        extend: 'print',
                        className: 'btn-primary',
                        text : '<i class="fas fa-print"></i>',
                        title: 'Comentarios',
                        messageTop: 'Impresión con la información de los Comentarios.',
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            columns: [0,1,2,3,4,5,6]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Comentarios',
                        titleAttr: 'EXCEL',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2,3,4,5,6]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-primary',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1,2,3,4,5,6]
                        }
                    },
                    {
                        extend: 'searchBuilder',
                        className: 'btn-primary',
                       
                    }                       
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}
	
    / ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/comentario.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#comentarioMod').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alert (error);
        })

    })

})