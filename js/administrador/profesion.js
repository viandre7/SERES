function modificar (id_profesion, profesion, id_nivel_educativo){
    console.log(id_profesion);
    $("#id").val(id_profesion);
    $("#txtCatMod").val(profesion);
    $("#Nivel").val(id_nivel_educativo);


}

function limpiar() {
    $("#txtNombreCat").val("");
    $("#imgCategoria").val("");
}

function eliminar(id_profesion, profesion, estado){
    $("#txtIdCatElm").val(id_profesion);
    $("#catEliminar").html(profesion);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
    }
   
    
$(document).ready(function(){
    buscar() 
    lol()   
    function lol(){

        $.ajax({
            url:'../../controlador/education.read.php',
            type:'GET',
            datatype:'JSON',
            data:null,

        }).done(function(json){
            console.log(json);
            var obj = JSON.parse(json);
            var datos2 ='';
            var datos ='';
            $.each(obj, function(key, value){
                datos2 += '<option value="'+value.id_nivel_educativo+'">'+value.nivel_educativo+'</option>';
                            })
                            
            $('#txtNivel').html(datos2);
            $('#Nivel').html(datos2);

                            
        }).fail(function(xhr,status,error){
            alert("error");
        })
    }

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/profesion.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class='table table-striped  compact table-hover row-border border-dark table-bordered dt-responsive nowrap'  style='font-size: 18px ' >";
                datos += '<thead>';
                    datos += '<tr>';
                        datos += '<th class="text-center align-middle border border-primary ">Estado</th>';
                        datos += '<th class="text-center align-middle border border-primary ">Profesion</th>';
                        datos += '<th class="text-center align-middle border border-primary ">Nivel Educativo</th>';
                        datos += '<th class="text-center align-middle border border-primary ">Modificar</th>';
                        datos += '<th class="text-center align-middle border border-primary ">Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                            if(value.estado=="A"){
                                datos += '<td class=" border border-primary activo text-wrap">Activo</td>'; 
                                datos += '<td class=" border border-primary activo text-wrap">'+value.profesion+'</td>'; 
                                datos += '<td class=" border border-primary activo text-wrap">'+value.nivel_educativo+'</td>'; 
                                datos += '<td class=" border border-primary activo text-center align-middle" ><a class="btn btn-primary" onclick="modificar('+value.id_profesion+',\''+value.profesion+'\',\''+value.id_nivel_educativo+'\',\''+value.estado+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class=" border border-primary activo text-center align-middle" ><a class="btn btn-danger" onclick="eliminar('+value.id_profesion+',\''+value.profesion+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                        
                            }else{
                                datos += '<td class=" border border-primary inactivo text-wrap">Inactivo</td>';
                                datos += '<td class=" border border-primary inactivo text-wrap">'+value.ocupacion+'</td>'; 
                                datos += '<td class=" border border-primary inactivo text-wrap">'+value.nivel_educativo+'</td>'; 
                                datos += '<td class=" border border-primary inactivo text-center align-middle" ><a class="btn btn-primary" onclick="modificar('+value.id_profesion+',\''+value.profesion+'\',\''+value.nivel_educativo+'\',\''+value.estado+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class=" border border-primary inactivo text-center align-middle" ><a class="btn btn-danger" onclick="eliminar('+value.id_ocupacion+',\''+value.ocupacion+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                        
                        
                            }
                          datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({  
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[1, 'asc']],   
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtip',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        title: 'SERES - Ocupacion ',
                        titleAttr: 'PDF',
                        className: 'btn-danger',
                        messageTop: 'ocupacion',
                        messageTop: 'PDF con la información de las categorias.',
                        footer: true,
                        autoFiltre : true ,
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            stripHtml: false, /* Aquí indicamos que no se eliminen las imágenes */
                            columns: [0,1]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                              } );
                        }
                    },
                    {
                        extend: 'print',
                        autoFiltre : true , 
                        footer: true,
                        text : '<i class="fas fa-print"></i>',
                        title: 'Ocupacion',
                        messageTop: 'Impresión con la información de las Ocupacion.',
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            stripHtml: false, /* Aquí indicamos que no se eliminen las imágenes */
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        footer: true,
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Ocupacion',
                        titleAttr: 'EXCEL',
                        className: 'btn-success',
                        exportOptions : {
                            stripHtml: true, /* Aquí indicamos que no se eliminen las imágenes */
                            columns: [0,1]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-info',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        footer: true,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1]
                        }
                    },
                    
                    'searchBuilder',                       
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}
   
    /// ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/profesion.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#categoriaEstado').serialize(),
            }).done(function(json){
                buscar();
                alertify.success(json);
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })

    })

})