function modificar (id_municipio, municipio, departamento,pais, codigo_dane){
    
    $("#txtIdMunicipio").val(id_municipio);
    $("#txtMunicipioMod").val(municipio);
    $("#txtCodigoDaneMod").val(codigo_dane);
    $("#txtPaisActual").val(pais);
    $("#txtDepartamentoAct").val(departamento);  
   
}

paisess()   
function paisess(){

    $.ajax({
        url:'../../controlador/pais.adm.read.php',
        type:'GET',
        datatype:'JSON',
        data:null,

    }).done(function(json){
        console.log(json);
        var obj = JSON.parse(json);
        var datos2 ='';
        var datos ='';

                datos2 += '<option> Seleccione un Pais</option>'; 
        $.each(obj, function(key, value){
            
            datos2 += '<option value="'+value.id_pais+'">'+value.pais+'</option>';
                        })
                        
        $('#txtNuevoPais').html(datos2);
                        
    }).fail(function(xhr,status,error){
        alert("error");
    })
}
$("#txtNuevoPais").change(function () {
    departa(this.value);
});
departa("")
function departa(id_pais){

    $.ajax({
        url:'../../controlador/departamento.adm.read.php',
        type:'GET',
        datatype:'JSON',
        data:null,

    }).done(function(json){
        console.log(json);
        var obj = JSON.parse(json);
        var datos2 ='';
        var filter = obj.filter(e=>e.id_pais == id_pais)
        console.log(filter);
             datos2 += '<option> Seleccione un Departamento</option>'; 
        $.each(filter, function(key, value){
           
            datos2 += '<option value="'+value.id_departamento+'">'+value.departamento+'</option>';
            
                        })
        $('#txtNuevoDep').html(datos2);
                        
    }).fail(function(xhr,status,error){
        alert("error");
    })
}


function limpiar() {
    $("#txtPaiss").val("");
    $("#txtDepartamentos").val("");
    $("#txtNombreMun").val("");
    $("#txtCodigoDane").val("");
}

function eliminar(id_municipio, municipio, departamento, estado, pais){
    $("#txtMunEli").val(id_municipio);
    $("#muniElim").html(municipio+' - '+departamento+' - '+pais);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
    }
    
$(document).ready(function(){
    buscar()
    pais()   
    function pais(){

        $.ajax({
            url:'../../controlador/pais.adm.read.php',
            type:'GET',
            datatype:'JSON',
            data:null,

        }).done(function(json){
            console.log(json);
            var obj = JSON.parse(json);
            var datos2 ='';
            var datos ='';

                    datos2 += '<option> Seleccione un Pais</option>'; 
            $.each(obj, function(key, value){
                
                datos2 += '<option value="'+value.id_pais+'">'+value.pais+'</option>';
                            })
                            
            $('#txtPaiss').html(datos2);
                            
        }).fail(function(xhr,status,error){
            alert("error");
        })
    }
    $("#txtPaiss").change(function () {
        departa(this.value);
    });
    departa("")
    function departa(id_pais){

        $.ajax({
            url:'../../controlador/departamento.adm.read.php',
            type:'GET',
            datatype:'JSON',
            data:null,

        }).done(function(json){
            console.log(json);
            var obj = JSON.parse(json);
            var datos2 ='';
            var filter = obj.filter(e=>e.id_pais == id_pais)
            console.log(filter);
                 datos2 += '<option> Seleccione un Departamento</option>'; 
            $.each(filter, function(key, value){
               
                datos2 += '<option value="'+value.id_departamento+'">'+value.departamento+'</option>';
                
                            })
            $('#txtDepartamentos').html(datos2);
                            
        }).fail(function(xhr,status,error){
            alert("error");
        })
    }

    // REGISTRAR ///
    $(document).on('click','#btnRegistrar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/municipio.adm.create.php',
                type: 'POST',
                dataType: 'json',
                data : $('#departamento').serialize(),
            }).done(function(json){
                alertify.success (json.mensaje);
                buscar();
               limpiar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/municipio.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class=' caption-top table  border-primary compact dt-responsive nowrap table-striped table-hover border rounded '>";
            datos += '<caption>Lista de Municipios</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="text-center border border-primary align-middle" >Estado</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Pais</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Departamento</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Municipio</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Codigo DANE</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Modificar</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                            if(value.estado=="A"){
                                datos += '<td class=" border border-primary activo text-wrap">Activo</td>'; 
                                datos += '<td class=" border border-primary activo text-wrap">'+value.pais+'</td>'; 
                                datos += '<td class=" border border-primary activo text-wrap">'+value.departamento+'</td>';
                                datos += '<td class=" border border-primary activo text-wrap">'+value.municipio+'</td>';
                                datos += '<td class=" border border-primary activo text-center align-middle">'+value.codigo_dep+''+value.codigo_dane+'</td>';
                                datos += '<td class=" border border-primary activo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_municipio+',\''+value.municipio+'\',\''+value.departamento+'\',\''+value.pais+'\',\''+value.codigo_dane+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class=" border border-primary activo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_municipio+',\''+value.municipio+'\',\''+value.departamento+'\',\''+value.estado+'\',\''+value.pais+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }else{
                                datos += '<td class=" border border-primary inactivo text-wrap">Inactivo</td>'; 
                                datos += '<td class=" border border-primary inactivo text-wrap">'+value.pais+'</td>'; 
                                datos += '<td class=" border border-primary inactivo text-wrap">'+value.departamento+'</td>';
                                datos += '<td class=" border border-primary inactivo text-wrap">'+value.municipio+'</td>';
                                datos += '<td class=" border border-primary inactivo text-center align-middle">'+value.codigo_dep+''+value.codigo_dane+'</td>';
                                datos += '<td class=" border border-primary inactivo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_municipio+',\''+value.municipio+'\',\''+value.departamento+'\',\''+value.pais+'\',\''+value.codigo_dane+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class=" border border-primary inactivo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_municipio+',\''+value.municipio+'\',\''+value.departamento+'\',\''+value.estado+'\',\''+value.pais+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }
                            datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({ 
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[1, 'asc']],    
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtip',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        className: 'btn-primary',
                        title: 'Municipios',
                        titleAttr: 'PDF',
                        messageTop: 'Municipios',
                        messageTop: 'PDF con la información de los Municipios.',
                        footer: true,
                        autoFiltre : true ,
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            columns: [0,1,2,3,4]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                             } );
                        }
                    },
                    {
                        extend: 'print',
                        text : '<i class="fas fa-print"></i>',
                        title: 'Municipios',
                        className: 'btn-primary',
                        messageTop: 'Impresión con la información de los Municipios.',
                        footer: true,
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            columns: [0,1,2,3,4]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Municipios',
                        titleAttr: 'EXCEL',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2,3,4]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-primary',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1,2,3,4]
                        }
                    },
                    
                    {
                        extend: 'searchBuilder',
                        className: 'btn-primary',
                       
                    }                        
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}
    /// MODIFICAR ///
    $(document).on('click','#btnModificar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/municipio.adm.update.php',
                type: 'POST',
                dataType: 'json',
                data : $('#municipioModificar').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })
	
    /// ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/municipio.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#municipioEstado').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })

    })

})