function modificar (id_pais, pais,codigo_dane){
    
    $("#txtIPaisMod").val(id_pais);
    $("#txtNombPaisMod").val(pais);
    $("#txtCodigoDaneMod").val(codigo_dane);
}

function limpiar() {
    $("#txtPais").val("");
    $("#txtCodigoDane").val("");
}

function eliminar(id_pais, pais, estado){
    $("#txtIdPaisElm").val(id_pais);
    $("#txtPaisEliminar").html(pais);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
    }
    
$(document).ready(function(){
    buscar(); 

    // REGISTRAR ///
    $(document).on('click','#btnRegistrar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/pais.adm.create.php',
                type: 'POST',
                dataType: 'json',
                data : $('#pais').serialize(),
            }).done(function(json){
                alertify.success (json.mensaje);
                buscar();
               limpiar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/pais.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class=' caption-top table  border-primary compact dt-responsive nowrap table-striped table-hover border rounded '>";
            datos += '<caption>Lista de Paises</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="text-center align-middle border border-primary " >Estado</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Pais</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Codigo DANE</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Modificar</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                            if(value.estado=="A"){
                                datos += '<td class=" border border-primary activo text-wrap">Activo</td>'; 
                                datos += '<td class=" border border-primary activo text-wrap">'+value.pais+'</td>'; 
                                datos += '<td class=" border border-primary activo text-center align-middle">'+value.codigo_dane+'</td>';
                                datos += '<td class=" border border-primary activo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_pais+',\''+value.pais+'\',\''+value.codigo_dane+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class=" border border-primary activo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_pais+',\''+value.pais+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }else{
                                datos += '<td class="border border-primary inactivo text-wrap">Inactivo</td>';
                                datos += '<td class="border border-primary inactivo text-wrap">'+value.id_pais+'</td>'; 
                                datos += '<td class="border border-primary inactivo text-center align-middle ">'+value.codigo_dane+'</td>';
                                datos += '<td class="border border-primary inactivo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_pais+',\''+value.pais+'\',\''+value.codigo_dane+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class="border border-primary inactivo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_pais+',\''+value.pais+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }
                            datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({ 
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[1, 'asc']],    
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtip',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        className: 'btn-primary',
                        title: 'Paises',
                        titleAttr: 'PDF',
                        messageTop: 'Paises',
                        messageTop: 'PDF con la información de los Paises.',
                        footer: true,
                        autoFiltre : true ,
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            columns: [0,1,2]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                             } );
                        }
                    },
                    {
                        extend: 'print',
                        text : '<i class="fas fa-print"></i>',
                        title: 'Paises',
                        className: 'btn-primary',
                        messageTop: 'Impresión con la información de los Paises.',
                        footer: true,
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Paises',
                        titleAttr: 'EXCEL',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-primary',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    
                    {
                        extend: 'searchBuilder',
                        className: 'btn-primary',
                       
                    }                     
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}
    /// MODIFICAR ///
    $(document).on('click','#btnModificar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/pais.adm.update.php',
                type: 'POST',
                dataType: 'json',
                data : $('#paisModificar').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })
	
    /// ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/pais.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#paisEstado').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })

    })

})