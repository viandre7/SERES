function modificar (id_departamento, departamento,pais, codigo_dane){
    
    $("#txtIdDepartamento").val(id_departamento);
    $("#txtDepartamentoMod").val(departamento);
    $("#txtPaisActual").val(pais);
    $("#txtCodigoDaneMod").val(codigo_dane);
    
    $.ajax({
        url:'../../controlador/pais.read.php',
        type: 'GET',
        dataType: 'json',
        data : $('#departamentoModificar').serialize(),
    }).done(function(json){
           var categorias = '';
        
                categorias += '<option> Seleccione un Pais o Confirme el Pais Actual</option>'; 
                $.each(json, function(key, value){
                        categorias += '<option value='+value.id_pais+'>'+value.pais+'</option>'; 
                })
        $('#txtNuevoPais').html(categorias);
    }).fail(function(xhr, status, error){
        $('#txtNuevoPais').html(error);
        console.log(error);
    });
   
}

function limpiar() {
    $("#txtNombreDeparta").val("");
    $("#txtPaiss").val("");
}

function eliminar(id_departamento, departamento, estado, pais){
    $("#txtDeparElm").val(id_departamento);
    $("#departElim").html(departamento+' - '+pais);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
    }
    
$(document).ready(function(){
    buscarPais();
    buscar(); 

    function buscarPais(){
        $.ajax({
            url:'../../controlador/pais.read.php',
            type: 'GET',
            dataType: 'json',
            data : $('#departamento').serialize(),
        }).done(function(json){
               var txtPais = '';
                     txtPais += '<option > Seleccione un Pais</option>'; 
                    $.each(json, function(key, value){
                        txtPais += '<option value='+value.id_pais+'>'+value.pais+'</option>'; 
                    })
            $('#txtPaiss').html(txtPais);
        }).fail(function(xhr, status, error){
            $('#txtPaiss').html(error);
        })
    }


    // REGISTRAR ///
    $(document).on('click','#btnRegistrar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/departamento.adm.create.php',
                type: 'POST',
                dataType: 'json',
                data : $('#departamento').serialize(),
            }).done(function(json){
                alertify.success (json.mensaje);
                buscar();
               limpiar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/departamento.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class=' caption-top table  border-primary compact dt-responsive nowrap table-striped table-hover border rounded '>";
            datos += '<caption>Lista de Departamentos</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="text-center border border-primary align-middle" >Estado</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Pais</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Departamento</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Codigo DANE</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Modificar</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                            if(value.estado=="A"){
                                datos += '<td class="border border-primary activo text-wrap">Activo</td>'; 
                                datos += '<td class="border border-primary activo text-wrap">'+value.pais+'</td>'; 
                                datos += '<td class="border border-primary activo text-wrap">'+value.departamento+'</td>';
                                datos += '<td class="border border-primary activo text-center align-middle>'+value.codigo_dane+'</td>';
                                datos += '<td class="border border-primary activo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_departamento+',\''+value.departamento+'\',\''+value.pais+'\',\''+value.codigo_dane+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class="border border-primary activo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_departamento+',\''+value.departamento+'\',\''+value.estado+'\',\''+value.pais+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }else{
                                datos += '<td class="border border-primary  inactivo text-wrap">Inactivo</td>';
                                datos += '<td class="border border-primary  inactivo text-wrap">'+value.pais+'</td>'; 
                                datos += '<td class="border border-primary  inactivo text-wrap">'+value.departamento+'</td>';
                                datos += '<td class="border border-primary  inactivo text-center align-middle">'+value.codigo_dane+'</td>';
                                datos += '<td class="border border-primary  inactivo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_departamento+',\''+value.departamento+'\',\''+value.pais+'\',\''+value.codigo_dane+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class="border border-primary  inactivo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_departamento+',\''+value.departamento+'\',\''+value.estado+'\',\''+value.pais+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }
                            datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({ 
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[1, 'asc']],    
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtip',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        className: 'btn-primary',
                        title: 'Departamentos',
                        titleAttr: 'PDF',
                        messageTop: 'Departamentos',
                        messageTop: 'PDF con la información de los Departamentos.',
                        footer: true,
                        autoFiltre : true ,
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            columns: [0,1,2,3]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                             } );
                        }
                    },
                    {
                        extend: 'print',
                        className: 'btn-primary',
                        text : '<i class="fas fa-print"></i>',
                        title: 'Departamentos',
                        messageTop: 'Impresión con la información de los Departamentos.',
                        footer: true,
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            columns: [0,1,2,3]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Departamentos',
                        titleAttr: 'EXCEL',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2,3]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-primary',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1,2,3]
                        }
                    },
                    {
                        extend: 'searchBuilder',
                        className: 'btn-primary',
                       
                    }                        
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}
    /// MODIFICAR ///
    $(document).on('click','#btnModificar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/departamento.adm.update.php',
                type: 'POST',
                dataType: 'json',
                data : $('#departamentoModificar').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })
	
    /// ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/departamento.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#departamentoEstado').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })

    })

})