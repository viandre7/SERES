function modificar (id_subcategoria, subcategoria,categoria){
    
    $("#txtIdSubcategoria").val(id_subcategoria);
    $("#txtNombresubcategoriaMod").val(subcategoria);
    $("#txtCategoriaActual").val(categoria);
    
    $.ajax({
        url:'../../controlador/categoria.read.php',
        type: 'GET',
        dataType: 'json',
        data : $('#subcategoriaModificar').serialize(),
    }).done(function(json){
           var categorias = '';
        
                 categorias += '<option >Seleccione una Categoria o Confirme la Categoria Actual </option>'; 
                $.each(json, function(key, value){
                        categorias += '<option value='+value.id_categoria+'>'+value.categoria+'</option>'; 
                })
        $('#txtNuevaCategoria').html(categorias);
    }).fail(function(xhr, status, error){
        $('#txtNuevaCategoria').html(error);
        console.log(error);
    });
   
}

function limpiar() {
    $("#txtNombreSubCat").val("");
    $("#txtCategorias").val("");
}

function eliminar(id_subcategoria, subcategoria, estado){
    $("#txtIdCatElm").val(id_subcategoria);
    $("#catEliminar").html(subcategoria);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
    }
    
$(document).ready(function(){
    buscarCategoria();
    buscar(); 

    function buscarCategoria(){
        $.ajax({
            url:'../../controlador/categoria.read.php',
            type: 'GET',
            dataType: 'json',
            data : $('#subcategoria').serialize(),
        }).done(function(json){
               var categorias = '';
            
                     categorias += '<option >Seleccione una Categoria </option>'; 
                    $.each(json, function(key, value){
                            categorias += '<option value='+value.id_categoria+'>'+value.categoria+'</option>'; 
                    })
            $('#txtCategorias').html(categorias);
        }).fail(function(xhr, status, error){
            $('#txtcategorias').html(error);
            console.log(error);
        })
    }


    // REGISTRAR ///
    $(document).on('click','#btnRegistrar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/subcategoria.adm.create.php',
                type: 'POST',
                dataType: 'json',
                data : $('#subcategoria').serialize(),
            }).done(function(json){
                alertify.success (json.mensaje);
                buscar();
               limpiar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/subcategoria.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class=' caption-top table  border-primary compact dt-responsive nowrap table-striped table-hover border rounded '>";
            datos += '<caption>Lista de Subcategorias</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="text-center border border-primary align-middle" >Estado</th>';
                        datos += '<th class="text-center border border-primary align-middle" >SubCategoria</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Categoria</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Modificar</th>';
                        datos += '<th class="text-center border border-primary align-middle" >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                            if(value.estado=="A"){
                                datos += '<td class="border border-primary activo text-wrap">Activo</td>'; 
                                datos += '<td class="border border-primary activo text-wrap">'+value.subcategoria+'</td>'; 
                                datos += '<td class="border border-primary activo text-wrap">'+value.categoria+'</td>';
                                datos += '<td class="border border-primary activo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_subcategoria+',\''+value.subcategoria+'\',\''+value.categoria+'\')" data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class="border border-primary activo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_subcategoria+',\''+value.subcategoria+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }else{
                                datos += '<td class="border border-primary inactivo text-wrap">Inactivo</td>';
                                datos += '<td class="border border-primary inactivo text-wrap">'+value.subcategoria+'</td>'; 
                                datos += '<td class="border border-primary inactivo text-wrap">'+value.categoria+'</td>';
                                datos += '<td class="border border-primary inactivo text-center align-middle"><a class="btn btn-primary" onclick="modificar('+value.id_subcategoria+',\''+value.subcategoria+'\',\''+value.categoria+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                                datos += '<td class="border border-primary inactivo text-center align-middle"><a class="btn btn-secondary" onclick="eliminar('+value.id_subcategoria+',\''+value.subcategoria+'\',\''+value.estado+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            
                            }
                            datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({ 
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[1, 'asc']],    
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtip',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        className: 'btn-primary',
                        title: 'Subcategorias',
                        titleAttr: 'PDF',
                        messageTop: 'Subcategorias',
                        messageTop: 'PDF con la información de las subcategorias.',
                        footer: true,
                        autoFiltre : true ,
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            columns: [0,1,2]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                             } );
                        }
                    },
                    {
                        extend: 'print',
                        text : '<i class="fas fa-print"></i>',
                        title: 'Subcategorias',
                        messageTop: 'Impresión con la información de las subcategorias.',
                        footer: true,
                        className: 'btn-primary',
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Subcategorias',
                        titleAttr: 'EXCEL',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-primary',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    
                    {
                        extend: 'searchBuilder',
                        className: 'btn-primary',
                       
                    }                       
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}
    /// MODIFICAR ///
    $(document).on('click','#btnModificar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/subcategoria.adm.update.php',
                type: 'POST',
                dataType: 'json',
                data : $('#subcategoriaModificar').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })
	
    /// ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/subcategoria.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#subcategoriaEstado').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })

    })

})