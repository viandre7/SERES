function modificar (id_documento , nombre, nomenclatura){
    $("#txttipoDocMod").val(id_documento );
    $("#txtdocmentMod").val(nombre);
    $("#txtNomeMod").val(nomenclatura);
}


function limpiar() {
    $("#txtTipdocumento").val("");
    $("#txtTipoDocumentoM").val("");
}

function eliminar(id_documento , nombre, estado, nomenclatura){
    $("#txttipoEli").val(id_documento );
    $("#txtTipdoElm").html(nombre + ' - ' +nomenclatura);
    if(estado=="A"){
        $("#estadoEliminar").html('Activo');
    }else{
        $("#estadoEliminar").html('Inactivo');
    }
}
    
$(document).ready(function(){
    buscar(); 

    // REGISTRAR ///
    $(document).on('click','#btnRegistrar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/tipodocumento.adm.create.php',
                type: 'POST',
                dataType: 'json',
                data : $('#unidadesmedida').serialize(),
            }).done(function(json){
                alertify.success (json.mensaje);
                buscar();
               limpiar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })

    /// BUSCAR ///
    function buscar (){
        $.ajax({
            url:'../../controlador/tipodocumento.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            var datos = '';
            datos += "<table id='tableCat' class=' caption-top table  border-primary compact dt-responsive nowrap table-striped table-hover border rounded '>";
            datos += '<caption>Lista de Tipo de Documentos</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="text-center align-middle border border-primary " >Estado</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Tipo Documento Iddentidad</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Nomenclatura</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Modificar</th>';
                        datos += '<th class="text-center align-middle border border-primary " >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        datos += '<tr>';
                        if(value.estado=="A"){
                            datos += '<td  class=" border border-primary activo text-wrap">Activo</td>';
                            datos += '<td  class=" border border-primary activo text-wrap">'+value.nombre+'</td>'; 
                            datos += '<td  class=" border border-primary activo text-center align-middle">'+value.nomenclatura+'</td>';   
                            datos += '<td class="  border border-primary activo text-center align-middle" ><a class="btn btn-primary" onclick="modificar('+value.id_documento +',\''+value.nombre+'\',\''+value.nomenclatura+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                            datos += '<td class="  border border-primary activo text-center align-middle" ><a class="btn btn-secondary" onclick="eliminar('+value.id_documento +',\''+value.nombre+'\',\''+value.estado+'\',\''+value.nomenclatura+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                    
                        }else{
                            datos += '<td  class="border border-primary inactivo text-wrap">Inactivo</td>';
                            datos += '<td  class="border border-primary inactivo text-wrap">'+value.nombre+'</td>'; 
                            datos += '<td  class="border border-primary inactivo text-center align-middle">'+value.nomenclatura+'</td>';  
                            datos += '<td  class="border border-primary inactivo text-center align-middle" ><a class="btn btn-primary" onclick="modificar('+value.id_documento +',\''+value.nombre+'\',\''+value.nomenclatura+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                            datos += '<td  class="border border-primary inactivo text-center align-middle" ><a class="btn btn-secondary" onclick="eliminar('+value.id_documento +',\''+value.nombre+'\',\''+value.estado+'\',\''+value.nomenclatura+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                    
                        }
                        datos += '</tr>';
                    })
                datos += '</tbody>';
            datos += '</table>';
            $('#datos').html(datos);
            $('#tableCat').DataTable({
                "lengthChange": false,
                "pageLength": 10,
                "paging": true,
                "ordering": true,
                "scrollX": true,
                "info": true,
                "order": [[1, 'asc']],    
                "destroy" : true,
                "autoWidth": false,
                "language": {"url": "//cdn.datatables.net/plug-ins/1.10.21/i18n/Spanish.json",
                            "buttons":{
                                copyTitle: 'Registros Copiados',
                                copySuccess:{
                                    _:'%d Registros Copiados',
                                    1:'1 Registro Copiado',
                                }
                            },
                            searchBuilder: {
                                add: 'Agregar',
                                data: 'Filtro',
                                clearAll: 'Limpiar',
                                title:{  0: 'Selecione Filtro',
                                _: 'Filtro(s) Seleccionado(s) (%d)'},
                                condition: 'Condición',
                                value: 'Valor',
                                deleteTitle: 'Suprimir',
                                leftTitle: 'Izquierda',
                                rightTitle: 'Derecha',
                                logicOr: 'O',
                                logicAnd: 'Y',
                                button: 'FILTRAR <i class="fas fa-filter"></i>',
                                conditions :{
                                    date: {
                                        after: 'Despues',
                                        before: 'Antes',
                                        between: 'Entre',
                                        empty: 'Vacío',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        notBetween: 'No entre',
                                        notEmpty: 'No vacío'
                                    },
                                    moment: {
                                        before: 'Antes',
                                        after: 'Despues',
                                        equals: 'Igual a',
                                        not: 'Diferente',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    number: {
                                        equals: 'Igual a',
                                        not: 'Diferente de',
                                        gt: 'Mayor que',
                                        gte: 'Mayor o igual que',
                                        lt: 'Menor que',
                                        lte: 'Menor o igual a',
                                        between: 'Entre',
                                        notBetween: 'No entre',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío'
                                    },
                                    string: {
                                        contains: 'Contiene',
                                        empty: 'Vacío',
                                        notEmpty: 'No vacío',
                                        equals: 'Es igual a',
                                        not: 'Diferente de',
                                        endsWith: 'Termina con',
                                        startsWith: 'Empieza por'
                                    },
                                }
                            },
                            },
                dom:  'Bfrtilp',
                buttons: [
                    {
                        extend: 'pdfHtml5',
                        orientation: 'landscape',
                        pageSize: 'LEGAL',
                        download: 'open',
                        title: 'Unidades de Medidas',
                        titleAttr: 'PDF',
                        className: 'btn-primary',
                        messageTop: 'Unidades de Medidas',
                        messageTop: 'PDF con la información de las Unidades de Medidas.',
                        text : '<i class="far fa-file-pdf"></i>',
                        exportOptions : {
                            columns: [0,1,2]
                        },
                        customize: function ( doc ) {
                            doc.content.splice( 1, 0, {
                                margin: [ 0, 0, 0, 12 ],
                                alignment: 'center',
                                image: 'data:image/png;base64,'
                              } );
                        }
                    },
                    {
                        extend: 'print',
                        text : '<i class="fas fa-print"></i>',
                        title: 'Unidades de Medidas',
                        messageTop: 'Impresión con la información de las Unidades de Medidas.',
                        titleAttr: 'IMPRIMIR',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'excelHtml5',
                        text : '<i class="fas fa-file-excel"></i>',
                        autoFiltre : true ,
                        title: 'Unidades de Medidas',
                        titleAttr: 'EXCEL',
                        className: 'btn-primary',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    {
                        extend: 'copyHtml5',
                        className: 'btn-primary',
                        text : '<i class="far fa-copy"></i>',
                        autoFiltre : true ,
                        titleAttr: 'COPIAR',
                        exportOptions : {
                            columns: [0,1,2]
                        }
                    },
                    
                    {
                        extend: 'searchBuilder',
                        className: 'btn-primary',
                       
                    }                    
                ]
            });
        }).fail(function(xhr, status, error){
            $('#datos').html(error);
    })
}

    /// MODIFICAR ///
    $(document).on('click','#btnModificar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/tipodocumento.adm.update.php',
                type: 'POST',
                dataType: 'json',
                data : $('#TipodocumentoMod').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alertify.error (error);
        })
    })

    
   // ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/tipodocumento.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#TipodocEstado').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
                alert (error);
        })

    })

})