function modificar (id_unidad_medida, nombre, nomenclatura){
    $("#txtUniMedMod").val(id_unidad_medida);
    $("#txtUnMedMod").val(nombre);
    $("#txtNomeMod").val(nomenclatura);
}
...
function eliminar(id_unidad_medida, nombre, estado, nomenclatura){
    $("#txtMunEli").val(id_unidad_medida);
    $("#txtUniMedElm").html(nombre + ' - ' +nomenclatura);
    if(estado=="A"){
...
$(document).ready(function(){
    ...
    $(document).on('click','#btnRegistrar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/unidadmedida.adm.create.php',
                type: 'POST',
                dataType: 'json',
                data : $('#unidadesmedida').serialize(),
            }).done(function(json){
    ...
    function buscar (){
        $.ajax({
            url:'../../controlador/unidadmedida.adm.read.php',
            type: 'POST',
            dataType: 'json',
            data : null,
        }).done(function(json){
            ...
            datos += '<caption>Lista de Unidades de Medidas</caption>';   
                datos += '<thead class="table-light">';
                    datos += '<tr  class="table-light border-primary ">';
                        datos += '<th class="border border-primary" >Estado</th>';
                        datos += '<th class="border border-primary" >Unidad de Medida</th>';
                        datos += '<th class="border border-primary" >Nomenclatura</th>';
                        datos += '<th class="border border-primary" >Modificar</th>';
                        datos += '<th class="border border-primary" >Cambiar Estado</th>';
                    datos += '</tr>';
                datos +=  '</thead>';
                datos += '<tbody>';
                    $.each(json, function(key, value){
                        ...
                        if(value.estado=="A"){
                            ...
                            datos += '<td  class=" border border-primary activo text-center align-middle">'+value.nomenclatura+'</td>';   
                            datos += '<td class="  border border-primary activo text-center align-middle" ><a class="btn btn-primary" onclick="modificar('+value.id_unidad_medida+',\''+value.nombre+'\',\''+value.nomenclatura+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                            datos += '<td class="  border border-primary activo text-center align-middle" ><a class="btn btn-secondary" onclick="eliminar('+value.id_unidad_medida+',\''+value.nombre+'\',\''+value.estado+'\',\''+value.nomenclatura+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                            ...
                            datos += '<td  class="  border border-primary inactivo text-center align-middle">'+value.nomenclatura+'</td>';  
                            datos += '<td  class="  border border-primary inactivo text-center align-middle" ><a class="btn btn-primary" onclick="modificar('+value.id_unidad_medida+',\''+value.nombre+'\',\''+value.nomenclatura+'\')"  data-bs-toggle="modal"  data-bs-target="#modalModificar"><i class="fas fa-edit"></i></a></td>';
                            datos += '<td  class="  border border-primary inactivo text-center align-middle" ><a class="btn btn-secondary" onclick="eliminar('+value.id_unidad_medida+',\''+value.nombre+'\',\''+value.estado+'\',\''+value.nomenclatura+'\')" data-bs-toggle="modal"  data-bs-target="#modalEliminar"><i class="fas fa-exchange-alt"></i></a></td>';
                ...
                buttons: [
                    {
                        ...
                        messageTop: 'Unidades de Medidas',
                        messageTop: 'PDF con la información de las Unidades de Medidas.',
                    ...
                    },
                    {
                        ...
                        title: 'Unidades de Medidas',
                        messageTop: 'Impresión con la información de las Unidades de Medidas.',
                        titleAttr: 'IMPRIMIR',
                        exportOptions : {
                            columns: [0,1,2]
                        }
    ...
    $(document).on('click','#btnModificar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/unidadmedida.adm.update.php',
                type: 'POST',
                dataType: 'json',
                data : $('#uniMedModificar').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
   ...
   // ELIMINAR ///
    $(document).on('click','#btnEliminar',function(event){
        event.preventDefault();
            $.ajax({
                url:'../../controlador/unidadmedida.adm.delete.php',
                type: 'POST',
                dataType: 'json',
                data : $('#UniMedEstado').serialize(),
            }).done(function(json){
                alertify.success(json);
                buscar();
            }).fail(function(xhr, status, error){
